...
do { \
	if (_inp == 5) { \
		_ctx.eop_coder_channel_speech_compression[_ch].i5 = VIN_SIG_COD_OUT00 + _coder; \
	} else if (_inp == 4) {\
		_ctx.eop_coder_channel_speech_compression[_ch].i4 = VIN_SIG_COD_OUT00 + _coder; \
	} else if (_inp == 3) { \
		_ctx.eop_coder_channel_speech_compression[_ch].i3 = VIN_SIG_COD_OUT00 + _coder; \
	} else if (_inp == 2) { \
		_ctx.eop_coder_channel_speech_compression[_ch].i2 = VIN_SIG_COD_OUT00 + _coder; \
	} else { \
		_ctx.eop_coder_channel_speech_compression[_ch].i1 = VIN_SIG_COD_OUT00 + _coder; \
	} \
} while (0)
...
extern int vin_utg_coefficients(struct vinetic_context *ctx, unsigned int ch);

#define vin_utg_coefficients_write(_ctx, _ch) \
({ \
	int __res; \
	__res = vin_utg_coefficients(&_ctx, _ch); \
	__res; \
})
